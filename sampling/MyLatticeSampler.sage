...
#        AA = [list(w) for w in list(B)]
#        #AA = matrix(RDF,[list(w) for w in list(B)])
#        #Q,R = qr(AA) # QR decomposition
#        Q,R = AA.QR() # QR decomposition
...
#        return [abs(RealField(prec)(R[i,i])) for i in range(n)], M_sage
        ...
        #BB = [vector(w) for w in list(B)]
        BB = [vector(B.column(i)) for i in range(n)]
        G,mu = gram_schmidt(BB)
        G = matrix(G)
        M_sage = Matrix([[RealField(prec)(G[i,j]) for i in range(n)] for j in range(n)])
        gs_norms = [M_sage.column(i).norm() for i in range(n)]
        return gs_norms,M_sage
