...
#acoeffs = [[F.random_element() if (i < p-1 or i >= 2*(p-1)) and (i < 3*(p-1) or i >= 4*(p-1)) else F(0) for i in range(OKq_deg)] for j in range(numsamples)]
...
# The Trace attack
print('Y_Trace_coeff attack beginning.')
successCnt = 0 
SUCCESS = False
...
for fac in fact:
    ...
    Frob_aa = [get_coef(aa,Yq,5)*x for aa in amodqlst]
    Frob_bb = [get_coef(bb,Yq,5)*x for bb in bmodqlst]
    ...
    for tt in Fqf:
        ...
        if chi2 > chi2_value:
            ...
            print('the guess of the coef(s0) is {}.'.format(s0_guess))
            if chi2 > chi2_record:
                chi2_record = chi2
                guess = t
                Tr_s0 = s0_guess
