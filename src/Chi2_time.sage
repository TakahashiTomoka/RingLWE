...
amodqlst = [_my_dot_product(c, OKqq_basis) for c in acoeffs]
bcoeffs = [[F.random_element() for i in range(f)] for _ in range(numsamples)]
bmodqlst = [_my_dot_product(c, OKqq_basis) for c in bcoeffs]
print("sampling finished")
